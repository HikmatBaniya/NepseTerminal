...
DEFAULT_SETTINGS = {
    ...
    "default_timeframe": "20d",
    "timeframes": ["5d", "10d", "20d", "40d", "60d", "120d", "250d"],
    "candles_limit": 120,
...
# -----------------------------
# Candle fetching (NEPSE)
# -----------------------------

# Map chart timeframe labels to trading-day window sizes for NEPSE daily candles.
_NEPSE_TF_DAYS = {
    "5d": 5, "10d": 10, "20d": 20, "40d": 40,
    "60d": 60, "120d": 120, "250d": 250,
}

class CandleFetcher:
    """
    Fetches NEPSE daily OHLC candles via nepse_data.NEPSEData.

    NEPSE only publishes daily candles, so the "timeframe" parameter is
    interpreted as a day-window label (e.g. "20d" = last 20 trading days).
    Returns candles oldest→newest as:
      [{"ts": int, "open": float, "high": float, "low": float, "close": float}, ...]
    where "ts" is an integer YYYYMMDD date key (for chart x-axis ordering).
    """

    def __init__(self):
        from nepse_data import NEPSEData
        self._nepse = NEPSEData()
        # key: (symbol, days, limit) -> (fetch_epoch, candles)
        self._cache: Dict[tuple, tuple] = {}
        self._cache_ttl_seconds: float = 60.0  # daily data changes slowly

    def get_klines(self, symbol: str, timeframe: str, limit: int = 120) -> List[dict]:
        """
        Returns up to *limit* daily OHLC candles for *symbol*.

        *timeframe* is a day-window label ("5d", "20d", "250d", …).
        Falls back to *limit* days if the label is unrecognised.
        """
        symbol = symbol.upper().strip()
        days = _NEPSE_TF_DAYS.get(timeframe, max(limit, 20))
        fetch_days = max(days, limit)

        now = time.time()
        cache_key = (symbol, fetch_days, limit)
        cached = self._cache.get(cache_key)
        if cached and (now - float(cached[0])) <= self._cache_ttl_seconds:
            return cached[1]

        try:
            raw = self._nepse.get_historical_ohlc(symbol, days=fetch_days)
        except Exception:
            return []
        ...
        for row in raw:
            try:
                # Convert "YYYY-MM-DD" date string to int key for chart ordering
                ts = int(str(row["date"]).replace("-", ""))
                candles.append({
                    "ts":    ts,
                    "open":  float(row["open"]),
                    "high":  float(row["high"]),
...
class PowerTraderHub(tk.Tk):
    ...
    def open_settings_dialog(self) -> None:
        ...
        tms_url_var = tk.StringVar(value=self.settings.get("tms_url", "https://tms58.nepsetms.com.np/"))
        ...
        add_row(r, "TMS URL:", tms_url_var); r += 1
        ...
        def save():
            try:
                ...
                self.settings["tms_url"] = tms_url_var.get().strip() or "https://tms58.nepsetms.com.np/"
